...
def train(args, snapshot_path):
    ...
    #model_s2.
    # 细病理模型
    model_s3 = VIM_seg(config, img_size=args.patch_size,
                     num_classes=args.num_classes+2, in_cha=3).cuda()
    model_s3.load_from(config)    
    ...
    # 优化器设置
    optimizer = optim.SGD(list(model_s1.parameters())+list(model_s2.parameters())+list(model_s3.parameters()), lr=base_lr,
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_s1 = loss_ce_s1 + loss_nc_s1 + loss_dice_s1
            ...
            mi_mask_s2 = torch.argmax(outputs_soft_s2, dim=1) == 3
            label_batch_s2 = encode_labels(label_batch, label_map = {0: 0, 1: 1, 2: 2, 3: 3, 4: 3})
            
            loss_ce_s2 = ce_loss_s2(outputs_s2, label_batch_s2[:].long())
            if iter_num > 0.3 * max_iterations:
                loss_dice_s2 = dice_loss_s2(outputs_soft_s2, label_batch_s2.unsqueeze(1), weight=[1,1,2,3])
            else:
                loss_dice_s2 = dice_loss_s2(outputs_soft_s2, label_batch_s2.unsqueeze(1), weight=[1,1,1,1])     
            loss_nc_s2 = nc_loss(volume_batch.repeat(1,3,1,1), outputs_soft_s2, label_batch_s2)
            loss_l1_s2 = F.l1_loss(myo_mask_s1.to(torch.float32), myo_mask_s2.to(torch.float32))
            loss_s2 = loss_ce_s2 + loss_l1_s2 + loss_nc_s2

            # stage 3
            a = outputs_soft_s2[:,2:,:,:]
            outputs_s3 = model_s3(torch.concat((volume_batch, outputs_soft_s2[:,2:,:,:]), dim=1))
            outputs_soft_s3 = torch.softmax(outputs_s3, dim=1)
            myo_mask_s3 = torch.argmax(outputs_soft_s3, dim=1) >= 2
            mi_mask_s3 = torch.argmax(outputs_soft_s3, dim=1) >= 3            
            label_batch_s3 = encode_labels(label_batch, label_map = {0: 0, 1: 1, 2: 2, 3: 3, 4: 4})
            
            loss_ce_s3 = ce_loss_s3(outputs_s3, label_batch_s3[:].long())
            if iter_num > 0.3 * max_iterations:
                loss_dice_s3 = dice_loss_s3(outputs_soft_s3, label_batch_s3.unsqueeze(1), weight=[1,1,3,3,3])
            else:
                loss_dice_s3 = dice_loss_s3(outputs_soft_s3, label_batch_s3.unsqueeze(1), weight=[1,1,1,1,1])            
            loss_nc_s3 = nc_loss(volume_batch.repeat(1,3,1,1), outputs_soft_s3, label_batch_s3)
            ...
            loss_s3 = loss_ce_s3 + loss_l1_s3_mi + loss_l1_s3_myo + loss_nc_s3
            ...
            writer.add_scalar('info/loss_dice', 2.5-loss_dice_s3, iter_num)
            logging.info(
                'iteration %d : loss : %f, loss_dice_s1: %f, loss_dice_s2: %f, loss_dice_s3: %f' %
                (iter_num, loss.item(), 1-loss_dice_s1.item(), 2-loss_dice_s2.item(), 2.5-loss_dice_s3.item()))
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_volume(sampled_batch["image"], sampled_batch["label"], model_s1, model_s2, model_s3, classes=num_classes+2, patch_size=args.patch_size)
