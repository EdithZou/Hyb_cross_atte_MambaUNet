...
class BaseDataSets(Dataset):
    def __init__(
        ...
        ops_strong=None,
        csv_file=None,
        slice_range = (3, 18) # 选择含有较多准确label信息的第3-18层的切片
    ):
        ...
        if split == "train":
            self.sample_list = data[data['subset'] == 'D8_postMI']
        else:
            # 你需要根据验证集或测试集的具体情况进行筛选
            self.sample_list = data[data['subset'] == 'val_subset_name']
...
class BaseDataSets(Dataset):
    def __init__(
        ...
        ops_weak=None,
        ops_strong=None,
        slice_range = (4, 16), # 选择含有较多准确label信息的第3-18层的切片
        random_state = 42,
        stage = "anatomy"
    ):
        self.split = split
    ...
    def _prepare_slices(self):
        if self.split == "train":
            for sample in self.sample_list:
                ...
                image_slices = image[:, :, start_slice:end_slice]
                label_slices = label[:, :, start_slice:end_slice]
            ...
            for sample in self.sample_list:
                ...
                if h > self.slice_range_height and w > self.slice_range_width:
                    ...
                    # 对每个切片进行裁切
                    img_volume = img_volume[h_start:h_end, w_start:w_end, 4:15]
                    lbl_volume = lbl_volume[h_start:h_end, w_start:w_end, 4:15]
