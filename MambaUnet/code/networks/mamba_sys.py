...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.d_conv = d_conv
        self.expand = expand
        self.d_inner = int(self.expand * self.d_model) # inner_dim = 2*dim
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        # 处理固定参数
        Ds = self.Ds.float().view(-1) # (k * d) = 4*192
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        self.cross_atte = CrossAttention(hidden_dim=24, num_heads=8)
        ...
        for i_layer in range(self.num_layers):
            concat_linear = nn.Linear(2*int(dims[0]*2**(self.num_layers-1-i_layer)),#线性层
            ...
            if i_layer ==0 :
                ...
                layer_up = DecoderLayer(
                    ...
                    dpr=dpr[sum(depths[:(self.num_layers-1-i_layer)]):sum(depths[:(self.num_layers-1-i_layer) + 1])],
                    norm_layer=norm_layer,
                    use_checkpoint=use_checkpoint
                )
                '''          
        ...
        if self.final_upsample == "expand_first":
            ...
            self.output_branch = nn.ModuleList([
                nn.Conv2d(in_channels=self.embed_dim,out_channels=1,kernel_size=1,bias=False)
                for _ in range(num_classes)
            ])
        self.apply(self._init_weights)
        #self.attention = nn.MultiheadAttention(embed_dim=self.embed_dim, num_heads=8, dropout=attn_drop_rate)
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            ...
            x = layer(x) # [B,H/4,W/4,96]->[B,H/8,W/8,192]->[B,H/16,W/16,384]->[B,H/32,W/32,768]
        x = self.norm(x)  # B H W C [B,3,3,768]
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x = torch.cat([x,x_downsample[3-inx]],-1) # skip connection
                x = self.concat_back_dim[inx](x)
                x = layer_up(x)
            # [B,H/32,W/32,768]->[B,H/16,W/16,384]->[B,H/8,W/8,192]->[B,H/4,W/4,96]
    ...
    def up_x4(self, x):
        if self.final_upsample=="expand_first":
            ...
            x = self.output(x)
            # 通过每个分支输出每个类的预测
            #outputs = [branch(x) for branch in self.output_branch]
        #return outputs  # 返回每个类的预测   
        return x
    
    def forward(self, x):
        x,x_downsample = self.forward_features(x) # [B,H/32,W/32,768]
        x = self.forward_up_features(x,x_downsample) # [B,H/4,W/4,96]
        x = self.up_x4(x) # [B,C,H,W]
        return x
...
class VSSM_attn(nn.Module):
    def __init__(self, patch_size=4, in_chans=1, num_classes=4, depths=[2, 2, 9, 2], 
        ...
        self.patch_embed = PatchEmbed2D(patch_size=patch_size, in_chans=in_chans, embed_dim=self.embed_dim,
            norm_layer=norm_layer if patch_norm else None)
        self.cross_atte = CrossAttention(hidden_dim=24, num_heads=8)
        self.seg_5c = nn.Conv2d(in_channels=self.num_classes, out_channels=1, kernel_size=1)
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
    ...
    def forward(self, x):
        x,x_downsample = self.forward_features(x) # [B,H/32,W/32,768]
        x, feats = self.forward_up_features(x,x_downsample) # [B,H/4,W/4,96]
        logits = self.up_x4(x) # [B,C,H,W]
        return x.permute(0, 3, 1, 2), feats.permute(0, 3, 1, 2), self.seg_5c(logits)
...
class DecoderLayer(nn.Module):
    def __init__(self, dim, depth, num_heads, drop_rate, attn_drop_rate, dpr, norm_layer, use_checkpoint):
        super().__init__()
        self.cross_attention = CrossAttention(dim, num_heads, attn_drop=attn_drop_rate)
        self.vss_layer_up = VSSLayer_up(
            dim=dim,
            depth=depth,
            drop=drop_rate,
            attn_drop=attn_drop_rate,
...
class TokenEmbedding(nn.Module):
    def __init__(self, num_classes, embed_dim):
        super().__init__()
        self.class_embeds = nn.Embedding(num_classes, embed_dim)

    def forward(self, class_ids):
        return self.class_embeds(class_ids)
