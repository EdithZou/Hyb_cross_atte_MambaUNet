...
_C.MODEL.PRETRAIN_CKPT = '/home/tzou/MyoSAIQ/Mamba_UNet/code/pretrained_ckpt/pretrained_ckpt/vmamba_tiny_e292.pth'
_C.MODEL.RESUME = ''
...
def _update_config_from_file(config, cfg_file, visited_files=None):
    ...
    try:
        ...
        print(f"Error loading YAML file {cfg_file}: {e}")
        return

    for cfg in yaml_cfg.setdefault('BASE', ['']):
        if cfg:
            cfg_path = os.path.join(os.path.dirname(cfg_file), cfg)
            if cfg_path in visited_files:
                print(f"Detected circular reference in config files: {cfg_path}")
                continue
            visited_files.add(cfg_path)
            print(f"Processing config file: {cfg_path}")
            _update_config_from_file(config, cfg_path, visited_files)

    print('=> merge config from {}'.format(cfg_file))
    try:
        config.merge_from_file(cfg_file)
    except Exception as e:
        print(f"Error merging config from {cfg_file}: {e}")
    config.freeze()
